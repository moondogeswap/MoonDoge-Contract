...
contract BootCamp is Ownable, ReentrancyGuard {
    ...
    // The address of the moon camp
    address public MOON_CAMP_ADDRESS;
    ...
    function initialize(
        ...
        require(msg.sender == MOON_CAMP_ADDRESS, "Not camp");
...
// File: contracts/MoonCamp.sol

pragma solidity 0.6.12;

contract MoonCamp is Ownable {
    event NewMoonCampContract(address indexed moonCamp);
     ...
     * @param _poolLimitPerUser: pool limit per user in stakedToken (if any, else 0)
     * @param _admin: admin address with ownership
     * @return address of new moon camp contract
     */
    function deployPool(
        IBEP20 _stakedToken,
        IBEP20 _rewardToken,
        ...
        bytes memory bytecode = type(BootCamp).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_stakedToken, _rewardToken, _startBlock));
        address moonCampAddress;

        assembly {
            moonCampAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        BootCamp(moonCampAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewMoonCampContract(moonCampAddress);
    }
}